defmodule TunezWeb.Artists.ShowLive do
  ...
  def handle_event("destroy-artist", _params, socket) do
    case Tunez.Music.destroy_artist(socket.assigns.artist) do
      :ok ->
        socket = socket |> put_flash(:info, "Artist deleted") |> push_navigate(to: ~p"/")
        {:noreply, socket}

      {:error, error} ->
        Logger.info("Shit went wrong")

        socket = socket |> put_flash(:error, "Error deleting artist, ups")
        {:noreply, socket}
    end
  end
  ...
  def handle_event("destroy-album", %{"id" => album_id}, socket) do
    case Tunez.Music.destroy_album(album_id) do
      ...
      :error ->
        socket = socket |> put_flash(:error, "No puede ser nooooo")
        {:noreply, socket}
    end
  end
